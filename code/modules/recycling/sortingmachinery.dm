	...
	if(istype(W, /obj/item/device/destTagger))
		...
		if(O.currTag)
			if(src.sortTag != O.currTag)
				...
				user << SPAN_WARN("The package is already labeled for [O.currTag].")
		else
			user << SPAN_WARN("You need to set a destination first!")
	...
	if (istype(target, /obj/item) && !(istype(target, /obj/item/storage) && !istype(target,/obj/item/storage/box)))
		...
		if (src.amount > 1)
			...
			user.visible_message(
				"\The [user] wraps \a [target] with \a [src].",\
