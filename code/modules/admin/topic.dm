...
//Regular jobs
	...
	//JOBBAN'S INNARDS
		...
		if(!check_rights(R_MOD,0) && !check_rights(R_BAN, 0))
			usr << SPAN_WARN("You do not have the appropriate permissions to add bans!")
			return
		...
		switch(alert("Temporary Ban?",,"Yes","No", "Cancel"))
			if("Yes")
				...
				if(check_rights(R_MOD, 0) && !check_rights(R_BAN, 0) && mins > config.mod_tempban_max)
					usr << SPAN_WARN("Moderators can only job tempban up to [config.mod_tempban_max] minutes!")
					return
