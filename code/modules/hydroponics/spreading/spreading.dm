...
/obj/effect/plant/attackby(var/obj/item/weapon/W, var/mob/user)
	...
	if(istype(W, /obj/item/weapon/wirecutters) || istype(W, /obj/item/weapon/scalpel))
		if(sampled)
			user << SPAN_WARN("\The [src] has already been sampled recently.")
			return
		if(!is_mature())
			user << SPAN_WARN("\The [src] is not mature enough to yield a sample yet.")
			return
		if(!seed)
			user << SPAN_WARN("There is nothing to take a sample from.")
			return
		if(sampled)
