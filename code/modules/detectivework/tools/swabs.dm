...
/obj/item/weapon/forensics/swab/attack(var/mob/living/M, var/mob/user)
	...
	if(H.wear_mask)
		user << SPAN_WARN("\The [H] is wearing a mask.")
		return

	if(!H.dna || !H.dna.unique_enzymes)
		user << SPAN_WARN("They don't seem to have DNA!")
		return
	...
	if(user.zone_sel.selecting == O_MOUTH)
		if(!H.organs_by_name[BP_HEAD])
			user << SPAN_WARN("They don't have a head.")
			return
		if(!H.check_has_mouth())
			user << SPAN_WARN("They don't have a mouth.")
			return
		...
		if(istype(O) && !O.is_stump())
			...
			if(istype(O) && !O.is_stump())
				has_hand = 1
		if(!has_hand)
			user << SPAN_WARN("They don't have any hands.")
			return
...
/obj/item/weapon/forensics/swab/afterattack(var/atom/A, var/mob/user, var/proximity)
	...
	if(is_used())
		user << SPAN_WARN("This swab has already been used.")
		return
	...
	var/choice
	if(!choices.len)
		user << SPAN_WARN("There is no evidence on \the [A].")
		return
	...
	if(choice == "Blood")
		...
		if(!istype(B) || !B.gunshot_residue)
			user << SPAN_WARN("There is no residue on \the [A].")
			return
