...
/obj/machinery/power/smes/buildable/attack_ai()
	if(RCon)
		...
		usr << SPAN_WARN("Connection error: Destination Unreachable.")
...
/obj/machinery/power/smes/buildable/attackby(var/obj/item/weapon/W as obj, var/mob/user as mob)
	...
	if (failing)
		user << SPAN_WARN("The [src]'s screen is flashing with alerts. It seems to be overloaded! Touching it now is probably not a good idea.")
		return
	...
	if (..())
		...
		if((charge > (capacity/100)) && safeties_enabled)
			user << SPAN_WARN("Safety circuit of [src] is preventing modifications while it's charged!")
			return

		if (output_attempt || input_attempt)
			user << SPAN_WARN("Turn off the [src] first!")
			return
		...
		if(istype(W, /obj/item/weapon/crowbar))
			if (terminal)
				user << SPAN_WARN("You have to disassemble the terminal first!")
				return

			playsound(get_turf(src), 'sound/items/Crowbar.ogg', 50, 1)
			user << SPAN_WARN("You begin to disassemble the [src]!")
			if (do_after(usr, 100 * cur_coils)) // More coils = takes longer to disassemble. It's complex so largest one with 5 coils will take 50s
				...
				usr << SPAN_WARN("You have disassembled the SMES cell!")
