		...
		if(istype(W, /obj/item/weapon/crowbar))
			if (charge < (capacity / 100))
				if (!output_attempt && !input_attempt)
					...
					user << SPAN_WARN("Turn off the [src] before dismantling it.")
			else
				user << SPAN_WARN("Better let [src] discharge before dismantling it.")
			...
			if (charge < (capacity / 100))
				...
				user << SPAN_WARN("Better let [src] discharge before putting your hand inside it.")
		else
