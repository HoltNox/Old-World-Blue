...
/obj/item/rig_module/proc/engage()
	...
	if(damage >= 2)
		usr << SPAN_WARN("The [interface_name] is damaged beyond use!")
		return 0

	if(world.time < next_use)
		usr << SPAN_WARN("You cannot use the [interface_name] again so soon.")
		return 0

	if(!holder || holder.canremove)
		usr << SPAN_WARN("The suit is not initialized.")
		return 0

	if(usr.lying || usr.stat || usr.stunned || usr.paralysis || usr.weakened)
		usr << SPAN_WARN("You cannot use the suit in this state.")
		return 0

	if(holder.wearer && holder.wearer.lying)
		usr << SPAN_WARN("The suit cannot function while the wearer is prone.")
		return 0
