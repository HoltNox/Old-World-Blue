...
/obj/item/rig_module/ai_container/proc/integrate_ai(var/obj/item/ai,var/mob/user)
	...
	if(ai_mob)
		...
		if(ai_mob.key && ai_mob.client)
			...
			user << SPAN_WARN("There is no active AI within \the [ai].")
	else
		user << SPAN_WARN("There is no active AI within \the [ai].")
	update_verb_holder()
	return
...
/obj/item/rig_module/datajack/accepts_item(var/obj/item/input_device, var/mob/living/user)
	...
	if(istype(input_device,/obj/item/weapon/disk/tech_disk))
		...
		if(disk.stored)
			if(load_data(disk.stored))
				...
				user << SPAN_WARN("The disk is corrupt. It is useless to you.")
		else
			user << SPAN_WARN("The disk is blank. It is useless to you.")
		return 1
	...
	// I fucking hate R&D code. This typecheck spam would be totally unnecessary in a sane setup.
		...
		if(!incoming_files || !incoming_files.known_tech || !incoming_files.known_tech.len)
			user << SPAN_WARN("Memory failure. There is nothing accessible stored on this terminal.")
		else
			// Maybe consider a way to drop all your data into a target repo in the future.
			if(load_data(incoming_files.known_tech))
				user << "<font color='blue'>Download successful; local and remote repositories synchronized.</font>"
			else
				user << SPAN_WARN("Scan complete. There is nothing useful stored on this terminal.")
		return 1
	return 0
