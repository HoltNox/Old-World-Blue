	...
	Topic(href, href_list)
		...
		if(href_list["jack"])
			...
			for(var/mob/M in viewers(T))
				M.show_message(SPAN_WARN("A port on [P] opens to reveal [P.cable], which promptly falls to the floor."), 3,
				               SPAN_WARN("You hear the soft click of something light and hard falling to the ground."), 2)
			return 1
