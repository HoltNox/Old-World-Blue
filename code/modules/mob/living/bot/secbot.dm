	...
	if(istype(M, /mob/living/carbon))
		...
		if(!cuff)
			...
			spawn(2)
				is_attacking = 0
				update_icons()
			visible_message(SPAN_WARN("[C] was prodded by [src] with a stun baton!"))
		else
			playsound(loc, 'sound/weapons/handcuffs.ogg', 30, 1, -2)
			visible_message(SPAN_WARN("[src] is trying to put handcuffs on [C]!"))
			if(do_mob(src, C, 60))
				if(!C.handcuffed)
		...
		spawn(2)
			is_attacking = 0
			update_icons()
		visible_message(SPAN_WARN("[M] was beaten by [src] with a stun baton!"))
