...
/material/proc/build_rod_product(var/mob/user, var/obj/item/stack/used_stack, var/obj/item/stack/target_stack)
	if(!rod_product)
		user << SPAN_WARN("You cannot make anything out of \the [target_stack]")
		return
	if(used_stack.get_amount() < 1 || target_stack.get_amount() < 1)
		user << SPAN_WARN("You need one rod and one sheet of [display_name] to make anything useful.")
		return
	used_stack.use(1)
	target_stack.use(1)
...
/material/proc/build_wired_product(var/mob/user, var/obj/item/stack/used_stack, var/obj/item/stack/target_stack)
	...
	if(used_stack.get_amount() < 5 || target_stack.get_amount() < 1)
		user << SPAN_WARN("You need five wires and one sheet of [display_name] to make anything useful.")
		return
...
/material/glass/build_windows(var/mob/living/user, var/obj/item/stack/used_stack)
	...
	if(!user.IsAdvancedToolUser())
		user << SPAN_WARN("This task is too complex for your clumsy hands.")
		return 1

	var/turf/T = user.loc
	if(!istype(T))
		user << SPAN_WARN("You must be standing on open flooring to build a window.")
		return 1
	...
	if(window_count >= 4)
		...
		if(choice in list("One Direction","Windoor"))
			...
			if(!failed_to_build && choice == "Windoor")
				if(!is_reinforced())
					user << SPAN_WARN("This material is not reinforced enough to use for a door.")
					return
				if((locate(/obj/structure/windoor_assembly) in T.contents) || (locate(/obj/machinery/door/window) in T.contents))
					failed_to_build = 1
	if(failed_to_build)
		user << SPAN_WARN("There is no room in this location.")
		return 1
	...
	if(used_stack.get_amount() < sheets_needed)
		user << SPAN_WARN("You need at least [sheets_needed] sheets to build this.")
		return 1
