...
//				detonator.loc=src.loc
		...
		if(stage > 1 && !active && clown_check(user))
			user << SPAN_WARN("You prime \the [name]!")
	...
	attackby(obj/item/weapon/W as obj, mob/user as mob)
		...
		if(istype(W,/obj/item/device/assembly_holder) && (!stage || stage==1) && path != 2)
			...
			if(istype(det.a_left,det.a_right.type) || (!isigniter(det.a_left) && !isigniter(det.a_right)))
				user << SPAN_WARN("Assembly must contain one igniter.")
				return
			if(!det.secured)
				user << SPAN_WARN("Assembly must be secured with screwdriver.")
				return
			path = 1
...
//					user << SPAN_WARN("You need to add at least one beaker before locking the assembly.")
				...
				if(active && prob(95))
					user << SPAN_WARN("You trigger the assembly!")
					prime()
					return
				else
			...
			path = 1
			if(beakers.len == 2)
				user << SPAN_WARN("The grenade can not hold more containers.")
				return
			else
