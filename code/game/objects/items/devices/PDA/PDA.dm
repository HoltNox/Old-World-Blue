		...
		if("Detonate")//Detonate PDA... maybe
			if(cartridge && cartridge.access_detonate_pda)
				...
				if(!(reception.message_server && reception.telecomms_reception & TELECOMMS_RECEPTION_SENDER))
					U.show_message(SPAN_WARN("An error flashes on your [src]: Connection unavailable"), 1)
					return
				if(reception.telecomms_reception & TELECOMMS_RECEPTION_RECEIVER == 0) // Does our recepient have a broadcaster on their level?
					U.show_message(SPAN_WARN("An error flashes on your [src]: Recipient unavailable"), 1)
					return
				if(!isnull(P))
					if (!P.toff && cartridge.charges > 0)
						cartridge.charges--
						...
						if(prob(difficulty))
							U.show_message(SPAN_WARN("An error flashes on your [src]."), 1)
						else if (prob(difficulty * 7))
							U.show_message(SPAN_WARN("Energy feeds back into your [src]!"), 1)
							ui.close()
							detonate_act(src)
		...
		switch(scanmode)
			if(1)
				...
				for (var/mob/O in viewers(C, null))
					O.show_message(SPAN_WARN("\The [user] has analyzed [C]'s vitals!"), 1)
			...
			if(4)
				for (var/mob/O in viewers(C, null))
					O.show_message(SPAN_WARN("\The [user] has analyzed [C]'s radiation levels!"), 1)
