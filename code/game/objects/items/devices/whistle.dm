...
obj/item/device/hailer/attack_self(mob/living/carbon/user as mob)
	...
	if(isnull(insults))
		...
		user.audible_message(
			SPAN_WARN("[user]'s [name] rasps, \"[use_message]\""),
			SPAN_WARN("\The [user] holds up \the [name].")
		)
	else
		if(insults > 0)
			playsound(get_turf(src), 'sound/voice/binsult.ogg', 100, 1, vary = 0)
			// Yes, it used to show the transcription of the sound clip.
			//  That was a) inaccurate b) immature as shit.
			user.audible_message(
				SPAN_WARN("[user]'s [name] gurgles something indecipherable and deeply offensive."),
				SPAN_WARN("\The [user] holds up \the [name].")
			)
			insults--
		else
