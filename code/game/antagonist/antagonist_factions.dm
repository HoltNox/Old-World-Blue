...
/mob/living/proc/convert_to_faction(var/datum/mind/player, var/datum/antagonist/faction)
	...
	if(faction.is_antagonist(player))
		src << SPAN_WARN("\The [player.current] already serves the [faction.faction_descriptor].")
		return

	if(player_is_antag(player))
		src << SPAN_WARN("\The [player.current]'s loyalties seem to be elsewhere...")
		return

	if(!faction.can_become_antag(player))
		src << SPAN_WARN("\The [player.current] cannot be \a [faction.faction_role_text]!")
		return
