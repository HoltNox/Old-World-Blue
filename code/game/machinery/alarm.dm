...
/obj/machinery/alarm/CanUseTopic(var/mob/user, var/datum/topic_state/state, var/href_list = list())
	...
	if(aidisabled && isAI(user))
		user << SPAN_WARN("AI control for \the [src] interface has been disabled.")
		return STATUS_CLOSE
	...
	switch(buildstage)
		if(2)
			...
			if (wiresexposed && istype(W, /obj/item/weapon/wirecutters))
				user.visible_message(SPAN_WARN("[user] has cut the wires inside \the [src]!"), "You have cut the wires inside \the [src].")
		...
		if(1)
			if(istype(W, /obj/item/stack/cable_coil))
				...
				if (C.use(5))
					...
					user << SPAN_WARN("You need 5 pieces of cable to do wire \the [src].")
					return
